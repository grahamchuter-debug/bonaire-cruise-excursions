/**
 * Bonaire request-to-book Worker (Classic Beach Day only).
 *
...
async function handlePreviewRequest(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message:
          "Live Stripe is not enabled for this Bonaire Worker. Charge-then-refund still requires an explicit production configuration. No payment was taken.",
      },
      503,
    );
  }
  ...
  const destinationBrand = destinationBrandFromCore(bonaireBookingCore);
