 ...
 */
/** Phase 20D: Graham-authorised live request-to-book unlock (Bonaire Island Sightseeing Tour). */
export const LIVE_PAYMENTS_CODE_ENABLED = true;

export const LIVE_UNLOCK_PHRASE = "BONAIRE_LIVE_UNLOCK";
...
export function liveCheckoutBlock(
  ...
  if (!LIVE_PAYMENTS_CODE_ENABLED) {
    return {
      ...
      message: "Live Stripe is not enabled for Bonaire. The live code flag is off. No payment was taken.",
    };
  }
  ...
  if (!site.includes("bonairecruiseexcursions.com")) {
    return {
      code: "SITE_BASE_URL",
      message: "Live Checkout requires SITE_BASE_URL on bonairecruiseexcursions.com. No payment was taken.",
    };
  }
  if (!product) {
    return {
