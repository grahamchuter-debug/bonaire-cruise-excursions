import { bonaireBookingCore } from "../../../../shared/destinations/bonaire";
...
function buildStripeLineItems(
  ...
  const bandLabel = (id: "adult" | "child" | "infant", fallback: string) =>
    product.ageBands.find((band) => band.id === id)?.label ?? fallback;
  const bands: Array<{ count: number; unitMajor: number | null; label: string }> = [
    { count: guests.adults, unitMajor: product.pricing.adultAmount, label: bandLabel("adult", "Participants age 3+") },
    { count: guests.children, unitMajor: product.pricing.childAmount, label: bandLabel("child", "Children") },
    { count: guests.infants, unitMajor: product.pricing.infantAmount, label: bandLabel("infant", "Children age 0–2") },
  ];
...
export async function handleCreateCheckout(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message: "Bonaire online booking is switched off. No payment was taken.",
      },
      503,
    );
  }
  ...
  const product = findBonaireBookingProduct(body.productId);
  ...
  const reference = createBookingReference(bonaireBookingCore);
  ...
  const metadata: Record<string, string> = {
    ...
    booking_worker: paymentsMode === "live" ? "bonaire-bookings-prod" : "bonaire-bookings-test",
