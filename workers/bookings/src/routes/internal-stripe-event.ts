...
export async function handleGatewayStripeEvent(request: Request, env: Env, ctx?: ExecutionContext): Promise<Response> {
  ...
  if (body.destination !== "bonaire") {
    return jsonResponse({ ok: false, code: "FOREIGN_DESTINATION", message: "Not a Bonaire event." }, 400);
  }
  ...
  if (body.product_id) {
    const product = findBonaireBookingProduct(body.product_id);
    if (!product) {
      return jsonResponse({ ok: false, code: "FOREIGN_PRODUCT", message: "Unknown product." }, 400);
    }
  }

  if (body.booking_reference && !/^W2BON-[A-Z0-9]+$/i.test(body.booking_reference)) {
  ...
  try {
    switch (body.event_type) {
      ...
      default:
        // Acknowledge unknown types without failing.
        break;
    }
